...
with focus_tab:
    ...
    if focus.empty:
        ...
        if decision_view.empty:
            st.info("No selections currently meet the focus threshold. That is a valid outcome. Do not force a bet.")
        else:
            for _, row in decision_view.head(8).iterrows():
                with st.container(border=True):
                    tier = str(row.get("DecisionTier", ""))
...
with pred_tab:
    st.subheader("Fixture-level model view")
...
with rank_tab:
    ...
    st.caption("Use this for detail after Decision Focus. Overall rank is a heuristic, not a literal probability.")
    if rankings.empty:
