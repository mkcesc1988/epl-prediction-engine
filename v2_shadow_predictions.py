...
def _preseason_form(history: pd.DataFrame, team: str, fixture_date: pd.Timestamp, cfg: dict, power_map: dict[str, float]) -> dict:
    vcfg = cfg.get("v2_shadow", {})
    ...
    raw_gds, weighted_gds, weights, known_flags = [], [], [], []
    for _, r in matches.iterrows():
        ...
        raw_gds.append(capped_gd); weighted_gds.append(capped_gd * weight); weights.append(weight); known_flags.append(1.0 if known else 0.0)
    weighted_avg = float(np.sum(weighted_gds) / np.sum(weights)) if np.sum(weights) > 0 else 0.0
    return {"matches": len(raw_gds), "raw_avg_gd": float(np.mean(raw_gds)), "weighted_avg_gd": float(np.clip(weighted_avg, -metric_cap, metric_cap)), "avg_opp_weight": float(np.mean(weights)), "known_opp_share": float(np.mean(known_flags))}
...
def _apply_context_adjustment(row: pd.Series, history: pd.DataFrame, cfg: dict, power_map: dict[str, float]) -> dict:
    fixture_date = pd.Timestamp(row["Date"]); home = str(row["HomeTeam"]); away = str(row["AwayTeam"])
    home_ctx = _context_features(history, home, fixture_date, cfg); away_ctx = _context_features(history, away, fixture_date, cfg)
    home_form = _preseason_form(history, home, fixture_date, cfg, power_map); away_form = _preseason_form(history, away, fixture_date, cfg, power_map)
    vcfg = cfg.get("v2_shadow", {})
    rest_component = float(vcfg.get("rest_log_lambda_per_day", 0.015)) * (_effective_rest(home_ctx["rest_days"], cfg) - _effective_rest(away_ctx["rest_days"], cfg))
    congestion_component = float(vcfg.get("congestion_log_lambda_per_extra_match", 0.020)) * (away_ctx["extra_congestion_matches"] - home_ctx["extra_congestion_matches"])
    form_component = float(np.clip(float(vcfg.get("preseason_goal_diff_log_lambda_per_goal", 0.018)) * (home_form["weighted_avg_gd"] - away_form["weighted_avg_gd"]), -float(vcfg.get("max_preseason_form_log_adjustment", 0.06)), float(vcfg.get("max_preseason_form_log_adjustment", 0.06))))
    log_adjustment = float(np.clip(rest_component + congestion_component + form_component, -float(vcfg.get("max_context_log_adjustment", 0.10)), float(vcfg.get("max_context_log_adjustment", 0.10))))
    base_home = float(row["Lambda_Home_xG"]); base_away = float(row["Lambda_Away_xG"])
    v2_home = base_home * math.exp(log_adjustment); v2_away = base_away * math.exp(-log_adjustment)
    matrix = _score_matrix(v2_home, v2_away, float(row.get("DixonColes_Rho", -0.08)), int(cfg.get("model_v12", {}).get("max_goal", 10)))
    markets = _derived_markets(matrix); best_idx = np.unravel_index(np.argmax(matrix), matrix.shape)
    return {
        "HomeRestDays": home_ctx["rest_days"], "AwayRestDays": away_ctx["rest_days"], "HomeMatchesLast14d": home_ctx["matches_in_window"], "AwayMatchesLast14d": away_ctx["matches_in_window"],
        "HomeExtraCongestionMatches": home_ctx["extra_congestion_matches"], "AwayExtraCongestionMatches": away_ctx["extra_congestion_matches"], "HomePreseasonMatches": home_form["matches"], "AwayPreseasonMatches": away_form["matches"],
        "HomePreseasonRawAvgGoalDiff": home_form["raw_avg_gd"], "AwayPreseasonRawAvgGoalDiff": away_form["raw_avg_gd"], "HomePreseasonAdjGoalDiff": home_form["weighted_avg_gd"], "AwayPreseasonAdjGoalDiff": away_form["weighted_avg_gd"],
        "HomePreseasonAvgOpponentWeight": home_form["avg_opp_weight"], "AwayPreseasonAvgOpponentWeight": away_form["avg_opp_weight"], "HomePreseasonKnownOpponentShare": home_form["known_opp_share"], "AwayPreseasonKnownOpponentShare": away_form["known_opp_share"],
        "RestComponentLogAdj": rest_component, "CongestionComponentLogAdj": congestion_component, "PreseasonFormLogAdj": form_component, "ContextLogAdjustment": log_adjustment,
        "V2_Lambda_Home_xG": v2_home, "V2_Lambda_Away_xG": v2_away, "V2_Lambda_Total_xG": v2_home + v2_away,
        "V2_P_HomeWin": markets["P_HomeWin_DC"], "V2_P_Draw": markets["P_Draw_DC"], "V2_P_AwayWin": markets["P_AwayWin_DC"], "V2_P_BTTS_Yes": markets["P_BTTS_Yes_DC"],
        "V2_P_Over2_5_Raw": markets["P_Over_2_5_DC"], "V2_P_Under2_5_Raw": 1.0 - float(markets["P_Over_2_5_DC"]), "V2_MostLikelyScore": f"{best_idx[0]}-{best_idx[1]}", "V2ContextAdjustmentApplied": abs(log_adjustment) > 1e-12,
    }


def _load_or_build_baseline(cfg: dict) -> pd.DataFrame:
    """Prefer a fresh V1.2 build, but do not let a transient provider outage kill V2."""
    try:
        return build_daily_predictions(cfg).copy()
    except Exception as exc:
        cached = Path(cfg["paths"]["processed_dir"]) / "daily_predictions_latest.csv"
        if not cached.exists():
            raise
        print(f"Fresh V1.2 build unavailable ({exc}). Reusing cached production predictions: {cached}")
        return pd.read_csv(cached)


def build_v2_shadow_predictions(cfg: dict) -> pd.DataFrame:
    shadow = _load_or_build_baseline(cfg)
    if shadow.empty:
        return shadow
    ...
    try:
        ...
        print(f"Fresh V2 history unavailable ({exc}). Continuing with empty league-history context; cached V1.2 predictions remain authoritative.")
    ...
    if not preseason.empty:
        ...
        print("No preseason/friendly context loaded. V2 will fall back to available league context.")
    ...
    shadow["BaselineModelVersion"] = shadow.get("ModelVersion", "V1.2"); shadow["ModelVersion"] = V2_VERSION; shadow["ShadowMode"] = True
    shadow["V2ValidationStatus"] = "Experimental opponent-adjusted preseason + rest/congestion context; not used for production bets"
    return shadow


def main() -> None:
    cfg = load_config(); out_dir = Path(cfg["paths"]["processed_dir"]); out_dir.mkdir(parents=True, exist_ok=True)
    shadow = build_v2_shadow_predictions(cfg); latest_path = out_dir / "v2_shadow_predictions_latest.csv"; shadow.to_csv(latest_path, index=False)
    print(f"Saved V2 shadow predictions: {latest_path}"); print("V1.2 production predictions and bet tracking were not modified.")
    if not shadow.empty:
        cols = ["Date", "HomeTeam", "AwayTeam", "HomeRestDays", "AwayRestDays", "HomePreseasonMatches", "AwayPreseasonMatches", "HomePreseasonRawAvgGoalDiff", "AwayPreseasonRawAvgGoalDiff", "HomePreseasonAdjGoalDiff", "AwayPreseasonAdjGoalDiff", "HomePreseasonAvgOpponentWeight", "AwayPreseasonAvgOpponentWeight", "ContextLogAdjustment", "P_HomeWin", "P_Draw", "P_AwayWin", "V2_P_HomeWin", "V2_P_Draw", "V2_P_AwayWin"]
